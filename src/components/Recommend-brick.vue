...
<script>
import { addCart } from "../util/api/getProduct";
...
export default {
  ...
  props: ["brick"],
  mounted() {
    this.img[0].src = require("../assets/images/product/product80/" +
      this.img[0].src);
    this.img[1].src = require("../assets/images/product/product80/" +
      this.img[1].src);
    this.img[2].src = require("../assets/images/product/product180/" +
      this.img[2].src);
  ...
  methods: {
    add_cart() {
      if (this.brick.pid && this.user_id) {
        addCart(this.brick.pid, this.user_id).then((data) => {
          if (data == 1) {
